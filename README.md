...
The read-only `mqtt_connected` property reports whether the MQTT client is currently active. It is `False` when startup falls back to API-only mode, after disconnects, and whenever the MQTT client loses its connection.
